...
const mutations = [
  ...
  { id: 'wrong-price', title: 'API returns the wrong price', kind: 'route-response', path: '/api/cart', method: 'GET', status: 200, body: '{"total":0,"currency":"USD"}' },
  { id: 'broken-image', title: 'Product image cannot decode', kind: 'break-image', path: '/hero.svg' },
  ...
  { id: 'cart-unavailable', title: 'Cart request is aborted', kind: 'route-abort', path: '/api/cart', method: 'GET' },
];
async function exercise({page, baseURL}) {
...
export function createDemoSuite(baseURL) {
  return defineSuite({ name: 'One checkout. Two very different test suites.', baseURL, repetitions: 2, timeoutMs: 7000,
    scenarios: [
...
export default createDemoSuite(process.env.FALSE_GREEN_BASE_URL ?? 'http://127.0.0.1:4177');
